...
def init_events(bot, cli_flags):
    ...
    async def on_voice_state_update(member, before, after):
        chat = 1031580662664282253
        war_room = 1033049367118106664
        kyle = 850206173873700914

        if member.id != kyle:
            return
        if after.channel is None:
            return
        if after.channel.id != war_room:
            return

        await bot.get_channel(chat).send(
            f"@everyone <@{kyle}> entered the <#{war_room}>"
        )
    ...
    async def on_command_error(ctx, error, unhandled_by_cog=False):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            msg = _(
                "`{user_input}` is not a valid value for `{command}`"
            ).format(user_input=error.user_input, command=error.cmd)
            ...
            if disabled_message:
                await ctx.send(
                    disabled_message.replace("{command}", ctx.invoked_with)
                )
            ...
            if not fuzzy_commands:
                ...
                await ctx.send(
                    embed=await format_fuzzy_results(
                        ctx, fuzzy_commands, embed=True
                    )
                )
            else:
                await ctx.send(
            ...
            if (
                bin(error.missing.value).count("1") == 1
            ):  # Only one perm missing
                msg = _(
            ...
            await ctx.send(
                _("That command is only available in NSFW channels.")
            )
            ...
            if delay := humanize_timedelta(seconds=error.retry_after):
                msg = _(
                    "This command is on cooldown. Try again in {delay}."
                ).format(delay=delay)
            else:
    ...
    async def on_message(message):
        ...
        if (
            ...
            or (discord_now - timedelta(minutes=60))
            > bot._checked_time_accuracy
        ):
    ...
    async def on_cog_add(cog: commands.Cog):
        ...
        for c in confs:
            ...
            await bot._config.custom("CUSTOM_GROUPS", c.cog_name, uuid).set(
                group_data
            )
